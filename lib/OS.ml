...
module OS = struct
  ...
  module Path = struct
    ...
    let rec toString { isAbs; vol; arcs } =
      begin if
        Bool.Bool.not isAbs
        && begin match arcs with "" :: _ -> true | _ -> false
        end
      then raise Path
      else begin
        if Bool.Bool.not (validVolume { isAbs; vol; arcs = [] }) then raise Path
        else begin
          if Bool.Bool.not (List.List.all validArc arcs) then raise InvalidArc
          else
            vol
            ^ begin if isAbs then "/" else ""
            end
            ^ String.String.concatWith "/" arcs
        end
      end
      end
    ...
    let rec getParent s =
      begin if s = "/" then s
        ...
        | Some (_, ".") -> s ^ "."
        | Some (_, "..") -> s ^ "/.."
        | Some ([], _) -> toString { isAbs; vol; arcs = [ "." ] }
    ...
    let rec splitDirFile s =
      let { isAbs; vol; arcs } = fromString s in
      begin match splitLast arcs with
      | None -> { dir = toString { isAbs; vol; arcs = [] }; file = "" }
      | Some (arcs, arc) -> { dir = toString { isAbs; vol; arcs }; file = arc }
      end

    let rec joinDirFile (p : dir_file) =
      let { isAbs; vol; arcs } = fromString p.dir in
      let arcs =
        begin if List.List.null arcs && p.file = "" then []
        else arcs @ [ p.file ]
        end
      in
    ...
    let rec splitArcBaseExt arc =
      let base_ss, ext_ss =
        Substring.Substring.splitr
          (fun c -> Bool.Bool.not (isExtSep c))
          (Substring.Substring.full arc)
      in
      begin if
        Substring.Substring.isEmpty ext_ss
        || Substring.Substring.size base_ss <= 1
      then None
      else
        Some
    ...
    let rec splitBaseExt s =
      let { isAbs; vol; arcs } = fromString s in
      begin match splitLast arcs with
      | None -> { base = toString { isAbs; vol; arcs = [] }; ext = None }
      | Some (arcs, arc) -> begin
          match splitArcBaseExt arc with
          | None -> { base = s; ext = None }
          | Some (b, e) ->
              {
                base = toString { isAbs; vol; arcs = arcs @ [ b ] };
                ext = Some e;
              }
        end
      end
    ...
    let rec mkCanonicalArcs (isAbs, arcs, arcs') =
      ...
      | _, ".." :: rest, ".." :: _ ->
          mkCanonicalArcs (true, rest, ".." :: ".." :: arcs')
      | _, ".." :: rest, _ :: arcs_rest ->
          mkCanonicalArcs (isAbs, rest, arcs_rest)
    ...
    let rec concat (s1, s2) =
      ...
      begin if p2.isAbs then raise Path
        ...
        if p2.vol = "" || p1.vol = p2.vol then
          toString
            {
              isAbs = p1.isAbs;
              vol = p1.vol;
              arcs = concatArcs (p1.arcs, p2.arcs);
            }
        else raise Path
      end
      end
    ...
    let rec mkAbsolute (p : abs_rel) =
      begin if isRelative p.relativeTo then raise Path
        ...
        if isAbsolute p.path then p.path
    ...
    let rec mkRelative (p : abs_rel) =
      ...
      let arcs1 =
        begin match p1.arcs with [ "" ] -> [] | _ -> p1.arcs
        end
      in
      begin if Bool.Bool.not p2.isAbs then raise Path
      else begin
        if Bool.Bool.not p1.isAbs then p.path
        else begin
          if Bool.Bool.not (p1.vol = p2.vol) then raise Path
          else begin
            match stripCommonPrefix (arcs1, p2.arcs) with
            | [], [] -> "."
            | arcs1', arcs2' ->
                toString
                  {
