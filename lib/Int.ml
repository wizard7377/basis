...
module Int = struct
  ...
  let rec div (a, b) =
    let q = a / b in
    begin if a mod b <> 0 && (a lxor b) < 0 then q - 1 else q end
  let rec mod_ (a, b) =
    let r = a mod b in
    begin if r <> 0 && (r lxor b) < 0 then r + b else r end
  ...
  let rec base = function
    | StringCvt.StringCvt.Bin -> 2 | StringCvt.StringCvt.Oct -> 8
    | StringCvt.StringCvt.Dec -> 10 | StringCvt.StringCvt.Hex -> 16
  ...
  and digit i =
    Char.Char.chr (i + (begin if i < 10 then Char.Char.ord '0' else Char.Char.ord 'A' - 10 end))

  and value c =
    Char.Char.ord (Char.Char.toUpper c) -
      (begin if c < 'A' then Char.Char.ord '0' else Char.Char.ord 'A' - 10 end)

  and fmt radix i =
    begin if i = 0 then "0"
    else begin if i > 0 then fmt_prime (base radix, -i, [])
  ...
  and scanHexPrefix getc src =
    ...
    bindOpt (getc src1) (fun (c2, src2) ->
      begin if c1 = '0' && (c2 = 'x' || c2 = 'X') then Some src2 else None end
    ))

  and scanPrefix radix getc src =
    begin if Bool.Bool.not (radix = StringCvt.StringCvt.Hex) then Some src
    else begin match scanHexPrefix getc src with
    | Some src' -> Some src'
    | None -> Some src
    end end

  and scanNum (isDigit_f, b) getc src =
    bindOpt (scanNum_prime (isDigit_f, b, 0, 0) getc src) (fun (i, k, src') ->
      begin if k > 0 then Some (i, src') else None end
    )
  and scanNum_prime (isDigit_f, b, i, k) getc src =
    begin match getc src with
    | Some (c, src') ->
      begin if isDigit_f c
      then scanNum_prime (isDigit_f, b, b * i - value c, k + 1) getc src'
      else Some (i, k, src)
      end
    | None -> Some (i, k, src)
    end
  ...
  let rec fromString s = StringCvt.StringCvt.scanString (scan StringCvt.StringCvt.Dec) s
  let ( > ) x__op y__op = x__op > y__op
  let ( >= ) x__op y__op = x__op >= y__op
  let ( < ) x__op y__op = x__op < y__op
  let ( <= ) x__op y__op = x__op <= y__op
...
module Int31 : INTEGER with type int = int = struct
  include Int
  type nonrec int = int
  let compare : int * int -> _ = assert false
end