...
module Time : TIME = struct
  ...
  let fromReal : float -> time =
   fun r ->
    begin if r < 0.0 then raise Time else r
    end
  ...
  let scan : (char, 'a) StringCvt.reader -> (time, 'a) StringCvt.reader =
   fun getc src ->
    ...
    | Some ('.', src') ->
        ...
        begin if has_whole || has_frac then begin
          match float_of_string_opt (whole ^ "." ^ frac) with
          | Some f -> Some (f, src'')
          | None -> None
        end
        else None
    ...
    | _ -> begin
        if has_whole then begin
          match float_of_string_opt whole with
          | Some f -> Some (f, src)
          | None -> None
        end
        else None
  ...
  let fromString : string -> time option = fun s -> StringCvt.scanString scan s
end
