...
module IO : OS_IO = struct
  ...
  let hash : iodesc -> int = fun fd -> Hashtbl.hash fd

  let compare : iodesc * iodesc -> order =
   fun (fd1, fd2) ->
    let h1 = Hashtbl.hash fd1 in
    let h2 = Hashtbl.hash fd2 in
    begin if h1 < h2 then Less
    else begin
      if h1 = h2 then Equal else Greater
    end
    end

  type iodesc_kind = File | Dir | Symlink | Tty | Pipe | Socket | Device

  let kind : iodesc -> iodesc_kind =
   fun fd ->
    ...
    | Unix.S_DIR -> Dir
    | Unix.S_LNK -> Symlink
    | Unix.S_CHR -> begin if Unix.isatty fd then Tty else Device end
    | Unix.S_FIFO -> Pipe
    | Unix.S_SOCK -> Socket
    | Unix.S_BLK -> Device
    end
  ...
  let pollDesc : iodesc -> poll_desc option =
   fun fd -> Some { pd_fd = fd; pd_in = false; pd_out = false; pd_pri = false }
  ...
  let poll : poll_desc list * Time.time option -> poll_info list =
   fun (pds, timeout) ->
    let read_fds =
      Stdlib.List.filter_map
        (fun pd ->
          begin if pd.pd_in then Some pd.pd_fd else None
          end)
        pds
    in
    let write_fds =
      Stdlib.List.filter_map
        (fun pd ->
          begin if pd.pd_out then Some pd.pd_fd else None
          end)
        pds
    in
    let except_fds =
      Stdlib.List.filter_map
        (fun pd ->
          begin if pd.pd_pri then Some pd.pd_fd else None
          end)
        pds
    in
    let t =
      begin match timeout with None -> -1.0 | Some time -> Time.toReal time
      end
    in
  ...
  let infoToPollDesc : poll_info -> poll_desc =
   fun pi ->
    {
      pd_fd = pi.pi_fd;
      pd_in = pi.pi_in;
