...
module StringCvt = struct
                     ...
                     let rec padding c i =
                       String.implode
                       (List.tabulate (clamp i, function 
                                                         | _ -> c));;
                     let rec padLeft c i s =
                       strCat (padding c (i - (String.size s)), s);;
                     let rec padRight c i s =
                       strCat (s, padding c (i - (String.size s)));;
                     ...
                     and splitl' (p, f, src, cs) =
                       ...
                       with 
                            ...
                            | Some (c, src') -> begin
                                ...
                                else (String.implode (List.rev cs), src) end
                       end;;
                     let rec takel p f s = let (a, _) = splitl p f s in a;;
                     let rec dropl p f s = let (_, b) = splitl p f s in b;;
                     let rec skipWS f s =
                       dropl
                       (function 
                                 | c
