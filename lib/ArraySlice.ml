...
module type ARRAY_SLICE = sig
  ...
  type nonrec 'a __0 = { src : 'a slice; dst : 'a Array.array; di : int }
...
module ArraySlice = struct
  ...
  let rec sub ((a, i, n), j) =
    begin if j < 0 || n <= j then raise Subscript else Array.sub (a, i + j)
    end

  let rec update ((a, i, n), j, x) =
    begin if j < 0 || n <= j then raise Subscript else Array.update (a, i + j, x)
    end
  ...
  and foldri' (f, x, sl, i) =
    begin if i = 0 then x
    else foldri' (f, f (i - 1, sub (sl, i - 1), x), sl, i - 1)
    end
  ...
  let rec modify f sl = modifyi (function _, a -> f a) sl
  ...
  let rec collate arg__0 arg__1 =
    ...
    | f, ((a1, i1, n1), (a2, i2, n2)) -> begin
        match f (Array.sub (a1, i1), Array.sub (a2, i2)) with
  ...
  let rec copy x =
    ...
    begin if di < 0 || Array.length dst < di + length src then raise Subscript
    else begin
      if src_start > di then copy_prime (src, dst, di, 0, length src, 1)
  ...
  let rec copyVec x =
    ...
    let dst : 'a Array.array = Obj.magic x.dst in
    let di = x.di in
    begin if di < 0 || Array.length dst < di + VectorSlice.length src then
      raise Subscript
