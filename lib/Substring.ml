...
module Substring = struct
  ...
  let rec substring (s, i, j) =
    try
      begin if i < 0 || j < 0 || String.size s < i + j then raise Subscript
      else (s, i, j)
      end
  ...
  let rec getc = function
    | s, i, 0 -> None
    | s, i, n -> Some (String.sub (s, i), (s, i + 1, n - 1))

  let first x = Option.map (function c, _ -> c) (getc x)

  let rec int_min_ (i, j) =
    begin if i < j then i else j
    end

  let rec int_max_ (i, j) =
    begin if i > j then i else j
    end

  let rec triml k (s, i, n) =
    begin if k < 0 then raise Subscript
    else (s, i + int_min_ (k, n), int_max_ (n - k, 0))
    end

  let rec trimr k (s, i, n) =
    begin if k < 0 then raise Subscript else (s, i, int_max_ (n - k, 0))
    end

  let rec slice = function
    | ss, i, Some m -> slice' (ss, i, m)
    | ss, i, None -> slice' (ss, i, size ss - i)

  and slice' ((s, i, n), j, m) =
    try
      begin if j + m > n then raise Subscript else substring (s, i + j, m)
      end
    with Overflow -> raise Subscript

  let concat x = String.concat (List.map string x)
  let rec concatWith s x = String.concatWith s (List.map string x)
  ...
  let rec collate f (ss, st) = String.collate f (string ss, string st)

  let rec splitAt ((s, i, n), j) =
    begin if j < 0 || j > n then raise Subscript
  ...
  and splitl' (f, s, i, n, j) =
    begin if j = i + n then ((s, i, n), (s, j, 0))
    else begin
      if f (String.sub (s, j)) then splitl' (f, s, i, n, j + 1)
      else ((s, i, j - i), (s, j, n - (j - i)))
    end
    end

  let rec splitr f (s, i, n) = splitr' (f, s, i, n, i + n - 1)

  and splitr' (f, s, i, n, j) =
    begin if j < i then ((s, i, 0), (s, i, n))
    else begin
      if f (String.sub (s, j)) then splitr' (f, s, i, n, j - 1)
      else ((s, i, j - i + 1), (s, j + 1, n - (j - i + 1)))
    end
    end

  let rec takel p ss =
    let a, _ = splitl p ss in
    a

  let rec dropl p ss =
    let _, b = splitl p ss in
    b

  let rec taker p ss =
    let _, b = splitr p ss in
    b

  let rec dropr p ss =
    let a, _ = splitr p ss in
    a

  let rec position s' (s, i, n) =
    let s, k, n' = position' s' (s, i, n) in
    ((s, i, k - i), (s, k, n'))

  and position' s' (s, i, n) =
    begin if String.size s' > n then (s, i + n, 0)
    else begin
      if isPrefix s' (s, i, n) then (s, i, n)
      else position' s' (triml 1 (s, i, n))
    end
    end

  let rec span ((s, i, n), (s', i', n')) =
    begin if Bool.not (s = s') || i' + n' < i then raise Span
    else (s, i, i' + n' - i)
    end
  ...
  and fields' (f, s, i, j, n) =
    begin if j = i + n then [ (s, i, j - i) ]
    else begin
      if f (String.sub (s, j)) then
        (s, i, j - i) :: fields' (f, s, j + 1, j + 1, n - 1)
      else fields' (f, s, i, j + 1, n - 1)
    end
    end

  let rec tokens f s =
    List.filter (function s, i, n -> Bool.not (n = 0)) (fields f s)
