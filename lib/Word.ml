...
module Word = struct
  ...
  and digit i =
    Char.Char.chr (begin if i < 10 then Char.Char.ord '0' + i else Char.Char.ord 'A' + i - 10 end)
  ...
  let rec scanPrefix radix getc src =
    ...
    bindOpt (getc src1) (fun (c2, src2) ->
      ...
      else begin if radix = StringCvt.StringCvt.Hex && (c2 = 'x' || c2 = 'X') then Some src2
      else begin if Bool.Bool.not (c2 = 'w') then None
      else bindOpt (getc src2) (fun (c3, src3) ->
        Some (begin if c3 = 'x' || c3 = 'X' then src3 else src2 end)
      )
      end end end
    ))
  ...
  and scanNum (isDigit_f, b) getc src =
    bindOpt (scanNum_prime (isDigit_f, b, 0, 0) getc src) (fun (w, k, src') ->
      begin if k > 0 then Some (w, src') else None end
    )
  and scanNum_prime (isDigit_f, b, w, k) getc src =
    begin match getc src with
    | Some (c, src') ->
      begin if not (isDigit_f c) then Some (w, k, src)
      else scanNum_prime (isDigit_f, b, b * w + fromInt (value c), k + 1) getc src'
      end
    | None -> Some (w, k, src)
    end

  and scan radix getc src =
    bindOpt (scanOptPrefix radix getc (StringCvt.StringCvt.skipWS getc src)) (fun src1 ->
