...
module Bool = struct
                ...
                let rec stringCvt_dropl p f src =
                  ...
                  with 
                       | None -> src
                       | Some (c, src') -> begin
                           if p c then stringCvt_dropl p f src' else src end
                  end;;
                let rec stringCvt_skipWS f s =
                ...
                let rec scanTextual (s, i) getc src = begin
                  ...
                  with 
                       | Some (c, src') -> begin
                           ...
                           scanTextual (s, i + 1) getc src' else None end
                       | None -> None
                  end end;;
                let rec scan getc src =
                  let src1 = stringCvt_skipWS getc src
                    in begin
