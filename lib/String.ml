...
module String = struct
                  ...
                  and explode' (s, i, l) = begin
                    if i < 0 then l else
                    explode' (s, i - 1, (sub (s, i) :: l)) end;;
                  let rec substring (s, i, j) =
                    try substring' (s, i, (i - 1) + j, [])
                    with 
                         | Overflow -> raise Subscript
                  and substring' (s, i, j, cs) = begin
                    if i > j then implode cs else
                    substring' (s, i, j - 1, (sub (s, j) :: cs)) end;;
                  let rec extract =
                    function 
