...
module Date : DATE = struct
  ...
  let date_of_tm (tm : Unix.tm) (off : Time.time option) : date =
    {
      ...
      d_offset = off;
      d_isDst =
        begin if tm.Unix.tm_isdst then Some true else Some false
        end;
    }

  let date : date__ -> date =
   fun d ->
    begin if d.month < 0 || d.month > 11 then raise Date
    else begin
      if d.day < 1 || d.day > 31 then raise Date
      else begin
        if d.hour < 0 || d.hour > 23 then raise Date
        else begin
          if d.minute < 0 || d.minute > 59 then raise Date
          else begin
            if d.second < 0 || d.second > 61 then raise Date
            else
              let tm =
                {
                  Unix.tm_sec = d.second;
                  tm_min = d.minute;
                  tm_hour = d.hour;
                  tm_mday = d.day;
                  tm_mon = d.month;
  ...
  let localOffset : unit -> Time.time =
   fun () ->
  ...
  let fromTimeLocal : Time.time -> date =
   fun t ->
    let tm = Unix.localtime (Time.toReal t) in
    date_of_tm tm None

  let fromTimeUniv : Time.time -> date =
   fun t ->
    let tm = Unix.gmtime (Time.toReal t) in
    date_of_tm tm (Some Time.zeroTime)

  let toTime : date -> Time.time =
   fun d ->
    let tm =
      {
        Unix.tm_sec = d.d_second;
        tm_min = d.d_minute;
        tm_hour = d.d_hour;
        tm_mday = d.d_day;
  ...
  let compare : date * date -> order =
   fun (d1, d2) -> Time.compare (toTime d1, toTime d2)
   ...
   fun fmtstr d ->
    ...
    let rec loop i =
      begin if i >= len then ()
        ...
        match Stdlib.String.get fmtstr i with
        | '%' when i + 1 < len ->
            ...
            | 'I' ->
                let h =
                  begin if d.d_hour = 0 then 12
                  else begin
                    if d.d_hour > 12 then d.d_hour - 12 else d.d_hour
                  end
                  end
                in
                Buffer.add_string buf (Printf.sprintf "%02d" h)
            | 'j' ->
                Buffer.add_string buf (Printf.sprintf "%03d" (d.d_yearDay + 1))
            | 'm' ->
                Buffer.add_string buf
                  (Printf.sprintf "%02d" (int_of_month d.d_month + 1))
            | 'M' -> Buffer.add_string buf (Printf.sprintf "%02d" d.d_minute)
            | 'p' ->
                Buffer.add_string buf
                  begin if d.d_hour < 12 then "AM" else "PM"
                  end
            | 'S' -> Buffer.add_string buf (Printf.sprintf "%02d" d.d_second)
            | 'Y' -> Buffer.add_string buf (Printf.sprintf "%04d" d.d_year)
            | 'y' ->
                Buffer.add_string buf (Printf.sprintf "%02d" (d.d_year mod 100))
            | '%' -> Buffer.add_char buf '%'
  ...
  let toString : date -> string = fun d -> fmt "%a %b %d %H:%M:%S %Y" d

  let scan : (char, 'a) StringCvt.reader -> (date, 'a) StringCvt.reader =
   fun _getc _src -> None

  let fromString : string -> date option = fun _s -> None
end
